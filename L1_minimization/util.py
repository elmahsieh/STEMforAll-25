...
def download_and_extract_cifar10(destination_path="../"):
    """
    Downloads and extracts the CIFAR-10 dataset if it doesn't exist.
    Returns the path to the extracted folder.
    """
    import urllib.request
    import tarfile

    cifar_folder = os.path.join(destination_path, "cifar-10-batches-py")
    cifar_url = "https://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz"
    tar_path = os.path.join(destination_path, "cifar-10-python.tar.gz")

    if os.path.exists(cifar_folder):
        print(f"✅ CIFAR-10 already exists at {cifar_folder}")
        return cifar_folder
...
def generate_report(original_image : np.ndarray,
    ...
    if recovery_time is not None:
        text += f"\nTime: {recovery_time:.2f} sec"
    plt.text(0.1, 0.5, text, fontsize=10)
